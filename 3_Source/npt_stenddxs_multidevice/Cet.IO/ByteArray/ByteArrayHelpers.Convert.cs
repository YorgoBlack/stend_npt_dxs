...
namespace Cet.IO
{
    ...
    public static partial class ByteArrayHelpers
    {
        ...
        public static float ReadSingle(ushort[] data, int offset)
        {
            byte[] asByte = new byte[] {
                ...
                (byte)(data[offset] / 256) };
            return BitConverter.ToSingle(asByte, 0);
        }

        public static void WriteSingle(ref ushort[] data, int offset, float value)
        {
            uint i = BitConverter.ToUInt32(BitConverter.GetBytes((float)value), 0);
            data[offset+1] = (ushort)i;
            data[offset] = (ushort)(i >> 16);
        }


        public static float ReadIEEE(ushort[] data, int offset)
        {
            byte[] asByte = new byte[] {
                (byte)(data[offset] % 256),
                (byte)(data[offset] / 256),
...
#pragma warning restore 3001, 3002
        ...
        public static float ReadIEEE754(
            ushort[] buffer,
            int offset)
        {
            byte[] arr = new byte[4];
            byte[] arr1 = BitConverter.GetBytes(buffer[offset]);
            byte[] arr2 = BitConverter.GetBytes(buffer[offset+1]);
            arr[0] = arr1[0];
            arr[1] = arr1[1];
            arr[2] = arr2[0];
            arr[3] = arr2[1];
